...
elseif ( 'mtypeedit' == $action ) {
	need_auth('admin');
	if ($id) {
		$mtype = Table::Fetch('mtype', $id);
		if (!$mtype) json('无数据', 'alert');
	}
	$html = render('manage_ajax_dialog_mtypeedit');
	json($html, 'dialog');
}
elseif ( 'mtyperemove' == $action ) {
	need_auth('admin');
	$mtype = Table::Fetch('mtype', $id);
	if (!$mtype) json('无此分类', 'alert');
	$tcount = Table::Count('movie', array('mtype' => $id));
	if ($tcount ) json('本类下存在电影项目', 'alert');
	Table::Delete('mtype', $id);
	option_mtype();
	Session::Set('notice', '删除分类成功');
	json(null, 'refresh');
}
else if ( 'movieremove' == $action) {
	need_auth('admin');
	Table::Delete('movie', $id);
	option_movie();
	/* remove coupon */
	Session::Set('notice', '删除电影成功');
	json(null, 'refresh');
}
