...
if ( is_get() && empty($news) ) {
	...
	$insert = array(
		'title', 
		'begin_time', 
		'detail','image','is_index','sort_order','display'
		);
	$news['image'] = upload_image('upload_image',$enews['image'],'team');
	if(empty($news['is_index']))  $news['is_index'] = 'N';
	if(empty($news['sort_order']))  $news['sort_order'] = 0;
	...
	if ( $news['id'] && $news['id'] == $id ) {
		...
		redirect( WEB_ROOT . "/manage/news/index.php#kk");
	} 
	else if ( $news['id'] ) {
