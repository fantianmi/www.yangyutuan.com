...
var URL = '/WhereReadyNews/index.php/Admin/Role';
...
<script language="javascript" type="text/javascript" src="/WhereReadyNews/statics/js/jquery/jquery.role.js"></script>
<div class="pad-lr-10">
    ...
    <table width="100%" cellspacing="0">
        ...
        <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td align="center" onclick="status(<?php echo ($val["id"]); ?>,'status')" id="status_<?php echo ($val["id"]); ?>"><img src="/WhereReadyNews/Public/Admin/images/status_<?php echo ($val["status"]); ?>.gif"</td>
...
function edit(id, name) {
	...
	window.top.art.dialog({title:lang_edit+'--'+name,id:'edit',iframe:'/WhereReadyNews/index.php/Admin/Role/edit/id/'+id,width:'400',height:'230'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
...
function status(id,type){
    $.get("<?php echo u('Role/status');?>", { id: id, type: type }, function(jsondata){
		...
		$("#"+type+"_"+id+" img").attr('src', '/WhereReadyNews/Public/Admin/images/status_'+return_data+'.gif');
	}); 
}
</script>
</body>
