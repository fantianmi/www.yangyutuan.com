...
class AdboardController extends BaseController
{
	function index()
	{
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Adboard/add').'\', title:\'添加广告位\', width:\'600\', height:\'300\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加广告位');
		$page = $p->show();
	...
	function add()
	{
		if(isset($_POST['dosubmit'])){
	    	...
	    	$adboard_mod->add();
	    	insertAdminLog('添加广告位--'.$data['name']);
	...
	function edit()
	{
		if(isset($_POST['dosubmit'])){
			...
			if(false !== $result){
				insertAdminLog('编辑广告位--id：'.$result);
	...
	function delete()
    {
		...
		if( isset($_POST['id'])&&is_array($_POST['id']) ){
			...
			insertAdminLog('删除广告位--ids：'.$ids);
			$adboard_mod->delete($ids);
		}else{
			$id = intval($_GET['id']);
			insertAdminLog('删除广告位--id：'.$id);
    ...
    {
        $type_files = glob(dirname($_SERVER['SCRIPT_FILENAME']) . '/Public/adboard/*.config.php');        
        $type_list = array();
