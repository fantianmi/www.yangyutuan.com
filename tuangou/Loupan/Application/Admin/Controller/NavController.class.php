...
class NavController extends BaseController{	
	...
	{		
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Nav/add').'\', title:\'添加导航\', width:\'540\', height:\'510\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);','添加导航');
	...
	function add()
	{
		if(isset($_POST['dosubmit'])){
			...
			insertAdminLog('添加导航--id：'.$app_cate_id);
	...
	function edit()
	{
		if(isset($_POST['dosubmit'])){
			...
			insertAdminLog('修改导航--id：'.$app_cate_id);
	...
	function delete()
	{
		...
		if (isset($_POST['id']) && is_array($_POST['id'])) {
			...
			insertAdminLog('删除导航--ids：'.$cate_ids);
			$this->nav_mod->delete($cate_ids);
		} else {
			$cate_id = intval($_GET['id']);
			insertAdminLog('删除导航--id：'.$cate_id);
