...
class NodeController extends BaseController
{
	...
	{	
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Node/add').'\', title:\'添加菜单\', width:\'500\', height:\'490\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加菜单');
		$page = $p->show();		
		$group_mod = D('group');
	...
	function delete()
	{
		...
		if (isset($_POST['id']) && is_array($_POST['id'])) {
			...
			insertAdminLog('删除菜单--ids：'.$ids);
			$node_mod->delete($ids);
		} else {
			$id = intval($_GET['id']);
			insertAdminLog('删除菜单--id：'.$id);
			$node_mod->delete($id);
		}
