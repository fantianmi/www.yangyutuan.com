...
class AdController extends BaseController
{
	function index()
	{
        ...
        $big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Ad/add').'\', title:\'新增广告\', width:\'600\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '新增广告');
	...
	function edit()
	{
		if(isset($_POST['dosubmit'])){
			...
			if(false !== $result){
				insertAdminLog('编辑广告--id：'.$data['id']);
	...
	function delete()
    {
		...
		if( isset($_POST['id'])&&is_array($_POST['id']) ){
			...
			insertAdminLog('删除广告--ids：'.$ids);
			$ad_mod->delete($ids);
		}else{
			$id = intval($_GET['id']);
			insertAdminLog('删除广告--id：'.$id);
