...
class AdminController extends BaseController
{
	function index()
	{
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Admin/add').'\', title:\'添加管理员\', width:\'480\', height:\'250\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加管理员');
		$page = $p->show();
	...
	function add()
	{
	    if(isset($_POST['dosubmit'])){
			...
			if($result){
				insertAdminLog('添加管理员：'.$_POST['user_name']);
	...
	function edit()
	{
		if(isset($_POST['dosubmit'])){
			...
			if(false !== $result){
				insertAdminLog('编辑管理员：'.$_POST['user_name'].'--id：'.$_POST['id']);
	...
	function delete()
	{
		...
		if (isset($_POST['id']) && is_array($_POST['id'])) {
		    ...
		    insertAdminLog('删除管理员--ids：'.$ids);
		    $admin_mod->delete($ids);
		} else {
			$id = intval($_GET['id']);
			insertAdminLog('删除管理员--id：'.$id);
			$admin_mod->delete($id);
		}
