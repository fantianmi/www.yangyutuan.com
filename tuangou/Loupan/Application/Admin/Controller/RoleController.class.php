...
class RoleController extends BaseController
{
	function index()
	{
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\''.U('Role/add').'\', title:\'添加角色\', width:\'400\', height:\'220\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加组');
		$page = $p->show();
	...
	function delete()
	{
		...
		if (isset($_POST['id']) && is_array($_POST['id'])) {
			...
			insertAdminLog('删除角色--ids：'.$ids);
			$role_mod->delete($ids);
		} else {
			$id = intval($_GET['id']);
			insertAdminLog('删除角色--id：'.$id);
			$role_mod->delete($id);
		}
	...
	public function authSubmit()
	{
		...
		insertAdminLog('角色授权--角色id：'.$role_id);
		$this->success(L('operation_success'));
	}
	//修改状态
	function status()
