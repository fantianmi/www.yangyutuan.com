...
if($gid!=24){
	...
	for($i = 0;$i<count($area);$i++){
		$area_condition=array('team_type'=>'normal','area_id'=>$area[$i][id], "begin_time < '{$now}'", "end_time > '{$now}'");
		if($gid!=0){
			$area_condition['group_id']=$gid;
		}
		if($sid!=0){
			$area_condition['sub_id']=$sid;
		}
	  $area[$i]['number'] = Table::Count('team', $area_condition);
	}
	...
	if($gid){
	   $cates = DB::LimitQuery('category', array('condition'=>array(
		...
		$hotmovies=DB::LimitQuery('movie',array('condition'=>array(
			'status'=>'2',
		),
		'order'=>'ORDER BY `id` DESC'
		));
		$comingmovies=DB::LimitQuery('movie',array('condition'=>array(
			'status'=>'1',
		),
		'order'=>'ORDER BY `id` DESC'
		));
		$theatercondition=array(
			'group_id'=>'2534',
			'display'=>'Y',
		);
		if($aid){
		  $theatercondition['area_id'] = $aid;
		}
		
		$hottheater=DB::LimitQuery('partner',array(
	...
	include template('category_movie');
}
