...
function option_mtype($force=false, $all=false){
	$cache = $force ? 0 : 86400*30;
	$mtypes = DB::LimitQuery('mtype', array(
		'cache' => $cache,
	));
	$mtypes = Utility::AssColumn($mtypes, 'id');
	return $all ? $mtypes : Utility::OptionArray($mtypes, 'id', 'name');
}
function option_movie($force=false, $all=false){
	$cache = $force ? 0 : 86400*30;
	$movies = DB::LimitQuery('movie', array(
		'cache' => $cache,
	));
	$movies = Utility::AssColumn($movies, 'id');
	return $all ? $movies : Utility::OptionArray($movies, 'id', 'name');
}
...
function get_team_price($partner_id){
	$team=DB::LimitQuery('team',array(
	'condition'=>array(
	'partner_id'=>2),
	'order'=>'ORDER BY `team_price` ASC',
	'offset'=>0,
	'size'=>2,
	));
	if($team[0]['team_price']){
		return $team[0]['team_price'];
	}else{
		return NOPRICE;
	}
}
