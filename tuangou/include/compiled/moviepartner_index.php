...
<div class="content">
	<div class="movie-container J-movie-container">
	    <div class="movie-container__body">
	<div class="movie-info movie-info--rich cf">
	    <div class="movie-info__cover">
	        <img src="<?php echo team_image($movie['image']); ?>" alt="<?php echo $movie['name']; ?>" width="166" height="234" >
	    </div>
	    <div class="movie-info__name">
	        <h2><?php echo $movie['name']; ?>&nbsp;&nbsp;<strong class="rates"><?php echo $movie['score_integer']; ?><strong class="rates-point">.<?php echo $movie['score_fractional']; ?></strong></strong></h2>
	    </div>
	    <i class="movie-info-version movie-info-version--2d"></i>
	    <section class="movie-info__body">
	        <dl class="movie-info__list"><dt>首映</dt><dd><?php echo date('Y.m.d',$movie['release']); ?></dd>
	        <dt>类型</dt>
	        <dd><?php echo $movie['mtype']; ?></dd>
	        <dt>版本</dt>
	        <dd><?php echo $movie['edition']; ?></dd>
	        <dt>地区</dt>
	        <dd><?php echo $movie['region']; ?></dd>
	        <dt>时长</dt><dd><?php echo $movie['duration']; ?></dd>
	        <dt>导演</dt>
	        <dd><?php echo $movie['director']; ?></dd><dt>主演</dt><dd><?php echo $movie['mainstar']; ?></dd>
	        </dl>
	    </section>
	</div>
    </div>
	    <div class="movie-container__foot">
	        <h4>剧情</h4>
	        <p id="J-short-movie-desc" class="desc"><?php echo $movie['plot']; ?></p>
	    </div>
	</div>
</div>
<div class="selector listpage" style="border:1px solid #dedede">
  ...
  <ul class="listblock lb2">
      <li><a href="movie.php?gid=<?php echo $gid; ?>&mid=<?php echo $movie['id']; ?>" class="<?php if(!$aid){?>focus<?php }?>">全部</a></li>
   <?php if(is_array($area)){foreach($area AS $index=>$one) { ?>
      <li><a href="movie.php?gid=<?php echo $gid; ?>&aid=<?php echo $one['id']; ?>&mid=<?php echo $movie['id']; ?>" class="<?php if($aid==$one['id']){?>focus<?php }?>"><?php echo $one['name']; ?><span><?php echo $one['number']; ?></span></a></li>
    <?php }}?>   
