                ...
                <div class="sect">
					...
					<?php if(is_array($orders)){foreach($orders AS $index=>$one) { ?>
						<tr <?php echo $index%2?'':'class="alt"'; ?>>
							...
							<td class="op"><?php if(($one['state']=='unpay'&&$teams[$one['team_id']]['close_time']==0)){?><a href="/order/check.php?id=<?php echo $one['id']; ?>">付款</a><?php } else if($one['state']=='pay') { ?><a href="/order/view.php?id=<?php echo $one['id']; ?>">详情</a>&nbsp;|&nbsp;<a href="/order/comment.php?id=<?php echo $one['id']; ?>"><?php echo get_comment_order($one['id'])?'已评':'点评'; ?></a><?php } else if($teams[$one['team_id']]['close_time']>0 ) { ?><a href="/ajax/order.php?action=orderdel&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="确定删除本订单？">删除</a><?php }?></td>
						</tr>
					<?php }}?>
