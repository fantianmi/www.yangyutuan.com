...
class Utils{
	...
	static function teamCreditPay($order){
		...
		$url=self::comhost();
		$dir=getAppDirName();
		$api = $url."/$dir/Api/func.php?token=".$token.'&type='.$type;
	...
	static function comhost(){
		$host=self::host();
		if (C('SERVER_COM')) {
			$comhost = 'http://'.getServerIp();
			$dir = getAppDirName();	
			$root=__ROOT__;
			$subfix=str_replace('/'.$dir,'',$root);
			return $comhost.$subfix;
		}else{
			return $host;
		}
	}
	...
	static function smsSend($phone,$content){
		...
		$token=urlencode(self::cfile($type));
		$dir=getAppDirName();
		$api = $host."/$dir/Api/func.php?token=".$token.'&type='.$type.'&tell='.urlencode($phone).'&text='.urlEncode($content);
		$res = zthttp_get($api);
		// var_dump($res);
...
//获取最土变量的绝对地址，例如LOG_PATH
    ...
    static function payNotifyUrl(){
    	$dir=getAppDirName();
    	return self::host()."/$dir/api.php";//新地址
    }
    ...
    static function buildUmspay(){
        ...
        $u->order_callback_url=self::payNotifyUrl();
