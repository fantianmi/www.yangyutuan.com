...
class TestAction extends CommonAction {
    ...
    public function AutoTestEnvironment(){
        ...
        echo 'CMSHOST:        ',Utils::comhost()?(Utils::comhost().(url_exists(Utils::comhost())?self::YES('  √'):self::NO('  x'))):self::iDie('获取失败'),'<br/>';
    	echo 'Site:		',Utils::siteName(),'<br/>';
        echo 'Ip:             ',getServerIp(),'<br/>';
    ...
    public function AutoTestModifi(){
    ...
    public function AutoTestCreateLinkToken(){
        ...
        for ($i=0; $i < $times; $i++) { 
            ...
            $url=Utils::comhost();
            $dir=getAppDirName();
            $api = $url."/$dir/Api/func.php?token=".urlencode($token).'&type='.$type;
    ...
    public function AutoTestPayState(){
        ...
        echo '------------<br/>';
        $wcEnable = WeChatPayFuns::isEnable();
        echo '微信支付:',($wcEnable)?' 开启':'   关闭','<br/>';
        if ($wcEnable){
                echo '商户号:',Utils::safeDisplayKey(C('wechatPayPartner'),false),'<br/>';
        }else{
            self::NO('未设置，请到Api/Conf/aonfig.php下填写');
        }
    }
    ...
    */
    public function AutoTestUmspayCreateOrder(){
        ...
        $ums->logDebug=true;
        $contentText = '仅88元享原价128元的美尔馨花卉特惠促销套餐精选红玫瑰11枝+百合花1枝！';
        $contentText = str_replace("+", ",", $contentText);
        $cont=$ums->order($contentText,1,'zttext-pay-x'.time());
        if ($cont) {
    ...
    中国测试银联
    */
    public function AutoTestUpmppayCreateOrder(){
        if (!UpmpHelper::isAvailable()) {
            return;
        }
        echo '<hr/>>>>中国测试银联下单接口测试<br/>';
        $err;
        $tn = UpmpHelper::fetchTradNumber('zttextd-'.time(),'juesttest',1,$err);
        if ($tn) {
            echo self::YES(' >下单成功:'.$tn),'<br/>';
        }else{
            echo self::NO(' >下单失败'),'<br/>';
            var_dump($err);
            echo "<br/>";
    ...
    public function AutoTestLogs(){
        ...
        if($admin){
            ...
            $logs=Utils::rlog('DEBUGLOG');
            if ($logs)echo 'debug-log<br/>';
            foreach($logs as $line){
                echo self::YES($line);
            }
        }else{
