...
//支付宝支付成功通知地址
    function notifyAlipay(){
	    ...
	    if(!$isVerify){
	    	...
	    	$base64=base64_encode(json_encode($_POST));
	    	Utils::ilog('支付宝-支付响应数据:'.$v_oid.':['.$base64.']','DEBUGLOG');
	    	die("fail");
	    }
	    //获取交易状态
...
//微信支付回调
	...
	//中国银联支付回调
	function notifyUpmppay(){
		$data=$_POST;
		//状态成功
		$bank = '中国银联';
		$v_oid = $this->_post('reqReserved');//商户系统的订单号,与请求一致

		if (UpmpHelper::verifySignature($_POST)){// 服务器签名验证成功
			//请在这里加上商户的业务逻辑程序代码
			//获取通知返回参数，可参考接口文档中通知参数列表(以下仅供参考)
			$transStatus = $_POST['transStatus'];// 交易状态
			if ("00"==$transStatus){
				// 交易处理成功
				$trade_no = $this->_post('orderNumber');
				$v_amount = Utils::moneyit($this->_post('settleAmount')*0.01);
				list($_, $order_id, $_, $_) = explode('-', $v_oid, 4);
				...
				$res=Utils::teamOtherPay($order_id, $v_oid, $v_amount, $currency, $service, $bank,$trade_no);
				if ($res===true) {
					Utils::log2db('中国银联回调','支付成功');
					echo 'success';
				}else{
					Utils::log2db('中国银联回调','支付失败');
					$lines='('.__CLASS__.':'.(__LINE__+1).')';
					Utils::ilog($bank.'-修改订单状态失败,订单ID:'.$v_oid.' '.$lines,'PAYERR');
					echo 'fail';
				}
