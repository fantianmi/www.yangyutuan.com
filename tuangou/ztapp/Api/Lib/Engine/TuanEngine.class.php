...
class TuanEngine extends BaseEngine{
	...
	function getUpmpStr($order_id){
		...
		$tn = UpmpHelper::fetchTradNumber($userOrder['pay_id'],$team['product'],$total_fee);
		if ($tn) {
			$json = array();
			$json['tn']=$tn;
			$json['mer_id']=upmp_config::mer_id();
			$tn=json_encode($json);
		}
        return $tn;
	}
		...
		if ($res['id']>0) {
			...
			if ($ress) {
				...
				if($ress['service']=='credit'){
		            ...
		            $dir=getAppDirName();
					$ress['pay_url']=$host."/$dir/api.php?s=/Tuan/pay&source=_client&id=".$index;
				}else{
					...
					$upmpPayStr=$this->getUpmpStr($ress['id']);
					if ($upmpPayStr) {//中国银联支付
						$ress['upmp_pay']=$upmpPayStr;
					}
				}
...
//获取推荐团购信息
function getRecommendList($key,$type,$city_id,&$count,&$page,$orderby,$lng,$lat,$partnerid,$coord=NULL,$ignore=NULL){
		$where=array();
		$subwhere=array();
		$key=trim($key);
		if ($key) {
...
//获取团购类型
	function tuanTypeList(){
		...
		return $newres===NULL?array():$newres;
	}
