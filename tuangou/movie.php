...
$mid=abs(intval($_GET['mid']));
$movie=Table::Fetch('movie',$mid);
...
for($i = 0;$i<count($area);$i++){
	$partner_condition=array();
	$partner_condition["area_id"]=$area[$i][id];
	$partner_condition["display"]='Y';
	if($group_id!=0){
		$partner_condition["group_id"]=$group_id;
	}
  	$area[$i]['number'] = Table::Count('partner', $partner_condition);
}


$category = Table::Fetch('category', $group_id);
$pagetitle = '品牌商户';
include template('moviepartner_index');
